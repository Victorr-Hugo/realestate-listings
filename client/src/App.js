...
function App() {
  return (
    <div className="App">
      <BrowserRouter>
        <Header />
        <div className="w-full h-full max-w-full max-h-full min-w-full min-h-full pt-16">
          <Routes>
            <Route path="/" element={<HomePage />} />
            <Route path="/:id" element={<ListingModal />} />
          </Routes>
        </div>
      </BrowserRouter>
    </div>
...
const HomePage = () => {
  const { posts, queryPosts, setPosts } = usePosts();
        ...
        <div className="h-full w-[321px] p-3 bg-white border-t text-left">
          ...
          <Formik
            ...
            onSubmit={async (values, actions) => {
              const res = await queryPosts(values);
              actions.resetForm();
    ...
    <header className="block w-full">
      <div className="m-0 w-full block relative">
        <div className="fixed z-[99999] flex top-0 right-0 bg-white shadow left-0 h-[66px] px-10 items-center min-w-full w-full my-0 mx-auto text-[#1b1b1b] flex-row box-border">
          ...
          <div className="w-full h-full relative">
            <div className="absolute hidden right-0 top-1/4 justify-center items-center flex-row md:flex font-medium text-[1rem]"></div>
          </div>
        </div>
      </div>
    </header>
